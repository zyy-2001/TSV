...
from llm_layers1 import add_tsv_layers
...
def main(): 
    ...
    parser.add_argument('--num_bottom', type=int, default=3)
    ...
    if args.gene:
        ...
        tsv = nn.ParameterList([
            nn.Parameter(torch.zeros(args.num_top + args.num_bottom, hidden_size), requires_grad=True)
            for _ in range(num_layers)
        ])

        tsv.to(device)
        add_tsv_layers(model, args.num_top, tsv, [args.lam], args)
